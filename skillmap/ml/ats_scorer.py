...
def score_keywords(
    text: str, spacy_skills: list[str], job_description: str = "", domains: list[dict] | None = None
) -> dict[str, Any]:
    lower = text.lower()
    ...
    if not jd_terms and domains and len(domains) > 0:
        ...
        if top_domain_key in POWER_SKILLS:
            domain_skills = list(
                set(s.lower() for s in _deep_flatten(POWER_SKILLS[top_domain_key]))
            )
...
def score_formatting(text: str) -> dict[str, Any]:
    ...
    if re.search(r"\[image\]|\[graphic\]|\.png|\.jpg|\.svg", text, re.I):
        pts -= 5
        issues.append("Image/graphic references detected")
    lines = text.split("\n")
    ...
    if fancy > 0:
        pts -= min(3, fancy)
        issues.append("Fancy bullets detected — use standard dashes")
    found_headers = sum(
        1 for kws in SECTION_KEYWORDS.values() if any(kw in text.lower() for kw in kws)
    )
    if found_headers < 3:
        pts -= 5
...
def score_action_verbs(text: str) -> dict[str, Any]:
    lines = [
        ...
        if re.match(r"^[-•–—*]", line.strip()) or (20 < len(line.strip()) < 200)
    ]
    if not lines:
...
def score_length(text: str) -> dict[str, Any]:
    wc = len(text.split())
    if 400 <= wc <= 700:
        pts, msg = 5, "Optimal length for a 1-page resume"
    elif 701 <= wc <= 900:
        pts, msg = 3, "Slightly dense — consider trimming"
    elif 901 <= wc <= 1200:
        pts, msg = 5, "Good length for a 2-page resume"
    elif wc > 1200:
        pts, msg = 2, "Too long — most ATS prefer 1-2 pages"
    elif wc < 250:
        pts, msg = 2, "Too short — add more detail"
    else:
        pts, msg = 3, "Acceptable length"
...
def detect_domains_nlp(
    ...
    for domain_key, subcategories in POWER_SKILLS.items():
        ...
        if domain_key == "Computer_Science_CSE":
            ...
            scores = {
                "MERN Stack Developer": len(
                    {"mongodb", "express", "react", "node.js", "nodejs"}.intersection(s_set)
                )
                * 1.5,
                "AI/ML Data Analyst": len(
                    {
                        "pandas",
                        "numpy",
                        "machine learning",
                ...
                "Cloud/DevOps Engineer": len(
                    {"aws", "docker", "kubernetes", "ci/cd", "terraform"}.intersection(s_set)
                ),
                "React Specialist": len({"react", "reactjs"}.intersection(s_set)),
                "Frontend Developer": len(
                    {"angular", "vue", "javascript", "html", "css", "tailwind"}.intersection(s_set)
                )
                * 0.5,
            ...
            sub_domain = best_match[0] if best_match[1] > 0 else "Software Engineer"
        ...
        # ECE Sub-domains
            ...
            scores = {
                "VLSI Engineer": len(
                    {"vlsi design", "verilog", "vhdl", "asic", "fpga"}.intersection(s_set)
                ),
                "IoT Specialist": len(
                    {"iot", "arduino", "raspberry pi", "sensors"}.intersection(s_set)
                ),
                "Embedded Systems Developer": len(
                    {"microprocessors", "embedded systems", "dsp", "c"}.intersection(s_set)
                ),
            }
            best_match = max(scores.items(), key=lambda x: x[1])
            sub_domain = best_match[0] if best_match[1] > 0 else "Electronics Engineer"

        # EEE Sub-domains
        elif domain_key == "Electrical_Engineering_EEE":
            s_set = set([s.lower() for s in matched_terms])
            scores = {
                "Power Systems Engineer": len(
                    {
                        "power systems",
                        "smart grid",
                        "renewable energy",
                ...
                "Power Electronics Specialist": len(
                    {"power electronics", "inverters", "converters", "motor drives"}.intersection(
                        s_set
                    )
                ),
                "Automation & Control Engineer": len(
                    {
                        "control systems",
                        "plc",
                        "scada",
            ...
            best_match = max(scores.items(), key=lambda x: x[1])
            sub_domain = best_match[0] if best_match[1] > 0 else "Electrical Engineer"
        ...
        # Mechanical Sub-domains
            ...
            scores = {
                "Design Engineer": len(
                    {"autocad", "solidworks", "catia", "ansys", "cad", "cam"}.intersection(s_set)
                ),
                "Manufacturing Engineer": len(
                    {"cnc", "gd&t", "lean manufacturing", "six sigma"}.intersection(s_set)
                ),
                "Thermal Engineer": len(
                    {"thermodynamics", "fluid mechanics", "heat transfer", "hvac"}.intersection(
                        s_set
                    )
                ),
            }
            best_match = max(scores.items(), key=lambda x: x[1])
            sub_domain = best_match[0] if best_match[1] > 0 else "Mechanical Engineer"

        # Civil Sub-domains
        elif domain_key == "Civil_Engineering":
            s_set = set([s.lower() for s in matched_terms])
            scores = {
                "Structural Engineer": len(
                    {"structural analysis", "staad.pro", "revit", "etabs"}.intersection(s_set)
                ),
                "Project Manager (Civil)": len(
                    {"construction management", "project estimation", "ms project"}.intersection(
                        s_set
                    )
                ),
            ...
            best_match = max(scores.items(), key=lambda x: x[1])
            sub_domain = best_match[0] if best_match[1] > 0 else "Civil Engineer"

        # Legal Sub-domains
        elif domain_key == "Legal":
            s_set = set([s.lower() for s in matched_terms])
            scores = {
                "Corporate Lawyer": len(
                    {"corporate law", "contract law", "compliance", "due diligence"}.intersection(
                        s_set
                    )
                ),
                "IP Attorney": len(
                    {"intellectual property", "patents", "trademarks"}.intersection(s_set)
                ),
                "Litigation Specialist": len(
                    {"litigation", "legal research", "case analysis"}.intersection(s_set)
                ),
            }
            best_match = max(scores.items(), key=lambda x: x[1])
            sub_domain = best_match[0] if best_match[1] > 0 else "Legal Professional"
        # Chemical Engineering Sub-domains
        elif domain_key == "Chemical_Engineering":
            s_set = set([s.lower() for s in matched_terms])
            scores = {
                "Process Engineer": len(
                    {"process control", "unit operations", "distillation"}.intersection(s_set)
                ),
                "Safety Engineer": len({"safety engineering", "hazop"}.intersection(s_set)),
                "Simulation Specialist": len(
                    {"aspen plus", "aspen hysys", "matlab"}.intersection(s_set)
                ),
            }
            best_match = max(scores.items(), key=lambda x: x[1])
            sub_domain = best_match[0] if best_match[1] > 0 else "Chemical Engineer"

        # Logistics & Supply Chain Sub-domains
        elif domain_key == "Logistics_Supply_Chain":
            s_set = set([s.lower() for s in matched_terms])
            scores = {
                "Supply Chain Manager": len(
                    {
                ...
                "Logistics Coordinator": len(
                    {"logistics", "warehousing", "distribution"}.intersection(s_set)
                ),
                "Procurement Specialist": len(
                    {"procurement", "vendor management", "sap scm"}.intersection(s_set)
                ),
            }
            best_match = max(scores.items(), key=lambda x: x[1])
            sub_domain = best_match[0] if best_match[1] > 0 else "Logistics Professional"

        # Sales Sub-domains
        elif domain_key == "Sales":
            s_set = set([s.lower() for s in matched_terms])
            scores = {
                "Business Development Manager": len(
                    {"business development", "lead generation", "sales strategy"}.intersection(
                        s_set
                    )
                ),
                "Account Executive": len(
                    {"account management", "relationship building", "negotiation"}.intersection(
                        s_set
                    )
                ),
                "Sales Specialist": len(
                    {"closings", "prospecting", "salesforce"}.intersection(s_set)
                ),
            }
            best_match = max(scores.items(), key=lambda x: x[1])
            sub_domain = best_match[0] if best_match[1] > 0 else "Sales Professional"

        # Aerospace Engineering Sub-domains
        elif domain_key == "Aerospace_Engineering":
            s_set = set([s.lower() for s in matched_terms])
            scores = {
                "Aerodynamics Engineer": len(
                    {"aerodynamics", "propulsion", "flight mechanics"}.intersection(s_set)
                ),
                "Avionics Specialist": len({"avionics", "orbital mechanics"}.intersection(s_set)),
                "Structural Dynamics Engineer": len(
                    {"structural mechanics", "ansys", "catia"}.intersection(s_set)
                ),
            }
            best_match = max(scores.items(), key=lambda x: x[1])
            sub_domain = best_match[0] if best_match[1] > 0 else "Aerospace Engineer"

        # Biomedical Engineering Sub-domains
        elif domain_key == "Biomedical_Engineering":
            s_set = set([s.lower() for s in matched_terms])
            scores = {
                "Medical Device Engineer": len(
                    {"medical devices", "biosensors", "biomaterials"}.intersection(s_set)
                ),
                "Rehabilitation Engineer": len(
                    {"rehabilitation engineering", "biomechanics"}.intersection(s_set)
                ),
                "Imaging Specialist": len(
                    {"medical imaging", "mimics", "labview"}.intersection(s_set)
                ),
            }
            best_match = max(scores.items(), key=lambda x: x[1])
            sub_domain = best_match[0] if best_match[1] > 0 else "Biomedical Engineer"

        # Industrial Engineering Sub-domains
        elif domain_key == "Industrial_Engineering":
            s_set = set([s.lower() for s in matched_terms])
            scores = {
                "Operations Analyst": len(
                    {
                        "operations research",
                        "production planning",
                ...
                "Quality Engineer": len(
                    {"quality control", "six sigma", "minitab"}.intersection(s_set)
                ),
                "Ergonomics Specialist": len(
                    {"ergonomics", "facilities design", "arena"}.intersection(s_set)
                ),
            }
            best_match = max(scores.items(), key=lambda x: x[1])
            sub_domain = best_match[0] if best_match[1] > 0 else "Industrial Engineer"

        # EEE Sub-domains
        elif domain_key == "Electrical_Engineering_EEE":
            s_set = set([s.lower() for s in matched_terms])
            scores = {
                "Power Electronics Engineer": len(
                    {"power electronics", "inverters", "converters", "smart grid"}.intersection(
                        s_set
                    )
                ),
                "Automation Engineer": len(
                    {"plc", "scada", "industrial automation"}.intersection(s_set)
                ),
            }
            best_match = max(scores.items(), key=lambda x: x[1])
            sub_domain = best_match[0] if best_match[1] > 0 else "Electrical Engineer"
        ...
        # Mechanical Sub-domains
            ...
            s_set = set([s.lower() for s in matched_terms])
            scores = {
                "Mechanical Design Engineer": len(
                    {"autocad", "solidworks", "catia", "3d modeling"}.intersection(s_set)
                ),
                "CAE Analyst": len({"ansys", "cfd", "fea"}.intersection(s_set)),
                "Manufacturing Engineer": len(
                    {"cnc", "gd&t", "lean manufacturing"}.intersection(s_set)
                ),
                "Thermal/HVAC Engineer": len(
                    {"thermodynamics", "hvac", "heat transfer"}.intersection(s_set)
                ),
            }
            best_match = max(scores.items(), key=lambda x: x[1])
            sub_domain = best_match[0] if best_match[1] > 0 else "Mechanical Engineer"
...
def generate_suggestions(
    cats: dict[str, dict[str, Any]], domains: list[dict] | None = None
) -> list[dict[str, Any]]:
    ...
    if kw["score"] < 15:
        add(
            "critical",
            "keywords",
            "Skills to Improve: Core Skills",
            f"Only {len(kw['matched'])} skills detected. Focus on adding: {', '.join(kw['missing'][:5])}.",
            "skills",
        )
    elif kw["score"] < 24:
        add(
            "important",
            "keywords",
            "Skills to Improve: Competitive Edge",
            f"To better match the industry benchmark, add: {', '.join(kw['missing'][:4])}.",
            "skills",
        )
    ...
    for issue in fmt.get("issues", [])[:2]:
        add(
            "critical" if fmt["score"] < 10 else "important",
            "formatting",
    ...
    if not d.get("email"):
        add(
            ...
            "Include a professional email address for ATS parsers.",
        )
    if not d.get("phone"):
        add(
            "critical",
            ...
            "A phone number is essential for recruiter outreach.",
        )
    if not d.get("linkedin") and domain_key in [
        "Computer_Science_CSE",
        "Marketing",
        "Sales",
        "Design_UX",
    ]:
        add(
            ...
            "For your domain, a LinkedIn profile significantly improves credibility.",
        )
    ...
    for m in struct.get("missing", []):
        add(
            "critical",
            "structure",
            f"Missing Section: {m.title()}",
            f'A dedicated "{m.title()}" section is required for optimal ATS parsing.',
        )
    ...
    if ach["count"] == 0:
        ...
        add(
            ...
            "achievements",
            "Strengthen Metrics",
            f"Detected {ach['count']} metrics. Aim for at least 5 quantified bullet points across your experience.",
            "experience",
        )
    ...
    if av.get("weak"):
        ...
        add(
            ...
            f'Replace passive terms like "{weak_str}" with high-impact verbs like "Orchestrated" or "Engineered".',
            "experience",
        )
    ...
    if domain_key == "Healthcare":
        add(
            ...
            "Ensure FHIR or HL7 standards are mentioned if relevant to your clinical tech role.",
        )
        ...
        add(
            ...
            "keywords",
            "FinTech Exposure",
            "Mention experience with RegTech or FinTech APIs to align with modern finance roles.",
        )
    elif domain_key == "Computer_Science_CSE":
        add(
            "important",
            "keywords",
            "System Architecture",
            "Include experience with design tokens or design systems if applying for frontend-heavy roles.",
        )
        add(
            "nice",
            "keywords",
            "Open Source",
            "Include a link to your GitHub profile or mention open-source contributions to demonstrate coding active-ness.",
        )
        ...
        add(
            ...
            "keywords",
            "ML Deployment",
            "Mention experience with model deployment and monitoring (MLOps) using tools like MLflow or Kubeflow.",
        )
        add(
            "important",
            "keywords",
            "Big Data",
            "Highlight experience with big data technologies like Spark or Hadoop if targeting enterprise data roles.",
        )
    ...
    if wc < 250:
        add(
            ...
            "length",
            "Content Depth",
            "Your resume is too brief. Expand on your responsibilities and technical projects.",
        )
    elif wc > 1200:
        add(
            "important",
            "length",
            "Conciseness",
            "Resume exceeds 1,200 words. Aim for a max of 2 pages (approx. 900-1000 words).",
        )
...
def score_resume(
    ...
    text_hash = hashlib.sha256((text + job_description).encode()).hexdigest()
