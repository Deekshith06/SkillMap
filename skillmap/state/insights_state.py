...
class InsightsState(AppState):
    ...
    def taxonomy_cluster_count(self) -> str:
        return str(self.stats.get("taxonomy_domains", 0))
    ...
    def skill_dist(self) -> list[dict]:
        ...
        for i, d in enumerate(cleaned_data[:10]):
            ...
            processed.append(
                {"index": i + 1, "skill": d["skill"], "count": count, "percent": f"{round(pct)}%"}
            )
        return processed
    ...
    def heatmap_points(self) -> list[dict]:
        ...
        for i, row_label in enumerate(labels):
            for j, col_label in enumerate(labels):
                ...
                points.append(
                    {"x": j, "y": i, "xLabel": col_label, "yLabel": row_label, "value": co}
                )
        return points

    @rx.var
    def scatter_points(self) -> list[dict]:
        """Lite mode has no 2D embedding artifact, so no scatter is returned."""
        return []
