...
def heatmap_cell(point: dict, max_val: int = 1) -> rx.Component:
    ...
    return rx.box(
        ...
        background_color=f"rgba(204, 69, 53, {intensity * 0.8 + 0.05})",
        title=f"{point.get('yLabel', '')} x {point.get('xLabel', '')}: {val}",
...
def insights_page() -> rx.Component:
    return rx.box(
        # Header
        rx.vstack(
            rx.heading("Insights", size="8", font_family=t.FONT_HEADING, color=t.DARK),
            rx.text(
                "Cluster distribution, skill analytics, and co-occurrence patterns.",
                color=t.SECONDARY,
            ),
            spacing="1",
            align_items="start",
        ...
        rx.box(
            rx.vstack(
                rx.heading(
                    "Skill Distribution", size="5", font_family=t.FONT_HEADING, color=t.DARK
                ),
        ...
        rx.box(
            rx.vstack(
                rx.heading(
                    "Skill Co-occurrence", size="5", font_family=t.FONT_HEADING, color=t.DARK
                ),
